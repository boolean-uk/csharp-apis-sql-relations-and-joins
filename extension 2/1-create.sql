...
CREATE TABLE actor (
  ...
  actor_fk_person INTEGER NOT NULL,
  CONSTRAINT fk_person FOREIGN KEY (actor_fk_person) REFERENCES person(person_id)
);

CREATE TABLE star (
  star_id SERIAL PRIMARY KEY,
  star_fk_actor INTEGER NOT NULL,
  star_dob DATE NOT NULL,
  CONSTRAINT fk_actor FOREIGN KEY (star_fk_actor) REFERENCES actor(actor_id)
);
